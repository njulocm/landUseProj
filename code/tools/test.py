...
# from crfrnn_layer_master.crfrnn.crf import CRF
import torch
...
def test_main(cfg):
    ...
    if test_cfg.is_predict:
        predict(is_crf= test_cfg.is_crf, model=model, dataset=dataset, out_dir=test_cfg.out_dir, device=device, batch_size=test_cfg.batch_size)
...
def predict(is_crf, model, dataset, out_dir, device, batch_size=128):
    '''
    输出预测结果
    :param model:
    :param dataset:

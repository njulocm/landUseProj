...
crf_transform = T.Compose([
    T_DL.DeNormalize_DL(mean=train_mean[:input_channel],
                       std=train_std[:input_channel]),
])

crf_train_transform = dict(
    train_transform=train_transform,
    crf_transform=crf_transform
)

crf_val_transform = dict(
    train_transform=val_transform,
    crf_transform=crf_transform
)

crf_test_transform = dict(
...
dataset_cfg = dict(
    ...
    train_transform=crf_train_transform,
    val_transform=crf_val_transform,
    test_transform=crf_test_transform,
)
...
model_cfg = dict(
    ...
    pretrained=True,
    crf_its = 5,
    train_crf_end2end=False,
    check_point_file=root_dir + '/code/checkpoint/smp_unetpp_cpu_crf_pretrain_b7_chnl4-rgb_argu_discolor-alltrain-se-0302/smp_unetpp_crf_best.pth',
...
train_cfg = dict(
    ...
    # optimizer_cfg=dict(type='adamw', lr=3e-13, momentum=0.99, weight_decay=0),
    # lr_scheduler_cfg=dict(policy='', T_0=5, T_mult=1, eta_min=1e-13),
    lr_scheduler_cfg=dict(policy='cos', T_0=3, T_mult=2, eta_min=1e-5),
    auto_save_epoch=5,  # 每隔几轮自动保存模型
    is_PSPNet=False,  # 不是PSPNet都设为false
    is_swa=False,
    check_point_file=root_dir + '/code/checkpoint/smp_unetpp_best_03985.pth',
)

test_cfg = dict(
    is_predict=True,  # 是否是预测分类结果
    is_evaluate=False,  # 是否评估模型，也就是计算mIoU
    is_trainingendcrf = False,
    tta_mode=None,  # 'd4'
    dataset='test_dataset',
    batch_size=train_cfg['batch_size'],
    num_workers=train_cfg['num_workers'],
    check_point_file=root_dir + '/code/checkpoint/smp_unetpp_noend2end_crf_a2-5b12r1-5_pretrain_b7_chnl4-rgb_argu_discolor-alltrain-se-0302/smp_unetpp_crf_best.pth',
    out_dir=root_dir + '/prediction_result/smp_unetpp_noend2end_crf_03970_lr-13_a2-5b11-5c1-5_pretrain_b7_chnl4-rgb_argu_discolor-alltrain-0301/',
)

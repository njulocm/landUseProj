...
class MessagePassing(nn.Module):
    ...
    def __init__(self, in_channels, n_classes, spatial_ker_weight=[3, ], bilateral_ker_weight=[3, ],
                 # theta_alpha=[2.0], theta_beta=[2.0], theta_gamma=[2.0]):
        ...
        for i in range(self.n_bilaterals):
            self.add_module( \
                'bilateral{}'.format(i), \
                BilateralFilter(in_channels, n_classes, bilateral_ker_weight[i], theta_alpha[i], theta_beta[i]))
        for i in range(self.n_spatials):
            self.add_module(
                'spatial{}'.format(i), SpatialFilter(n_classes, spatial_ker_weight[i], theta_gamma[i]))
...
class CRFRNN(nn.Module):
    ...
    def __init__(self, n_iter, in_channels, n_classes, spatial_ker_weight=[3, 3], bilateral_ker_weight=[3, 3],
                 theta_alpha=[1.5, 2.5], theta_beta=[11.5, 12.0], theta_gamma=[1.5, ]):
        ...
        self.messagepassing = MessagePassing(
            in_channels, n_classes, spatial_ker_weight=spatial_ker_weight, bilateral_ker_weight=bilateral_ker_weight,
    ...
    def forward(self, U, I, mode='train'):
        Q = U
        n_iter = 10 if mode == "test" else self.n_iter

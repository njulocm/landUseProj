...
class HRNetV2(nn.Module):
    def __init__(self, n_class, **kwargs):
        ...
        self.last_layer = nn.Conv2d(720, n_class, kernel_size=1, stride=1, padding=0,
                               bias=False)
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        x0 = F.interpolate(
            x[0], size=(ori_h, ori_w), mode='bilinear', align_corners=False)
        x1 = F.interpolate(
            x[1], size=(ori_h, ori_w), mode='bilinear', align_corners=False)
        x2 = F.interpolate(
            x[2], size=(ori_h, ori_w), mode='bilinear', align_corners=False)
        x3 = F.interpolate(
            x[3], size=(ori_h, ori_w), mode='bilinear', align_corners=False)

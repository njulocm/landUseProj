...
class DRN(nn.Module):
    ...
    def __init__(self, block, layers,input_channel=3, arch='D',
        ...
        if arch == 'C':
            self.conv1 = nn.Conv2d(input_channel, channels[0], kernel_size=7, stride=1,
            ...
            self.layer0 = nn.Sequential(
                nn.Conv2d(input_channel, channels[0], kernel_size=7, stride=1, padding=3,
                          bias=False),
...
def drn_d_54(BatchNorm,input_channel=3, pretrained=True):
    model = DRN(Bottleneck, [1, 1, 3, 4, 6, 3, 1, 1],input_channel, arch='D', BatchNorm=BatchNorm)
    if pretrained:
